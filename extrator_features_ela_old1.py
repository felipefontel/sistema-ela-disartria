...
def get_audio_path(recording):
    """
    Função Helper Principal: Prepara o arquivo de áudio para leitura e APLICA NORMALIZAÇÃO DE PICO.
    Garante que o áudio, seja local ou na nuvem, possua um volume de padronização universal (0.99)
    para evitar discrepâncias nos cálculos dos algoritmos acústicos (Garbage In, Garbage Out).
    """
    if not recording or not recording.audio_file:
        return None
        
    try:
        # TENTATIVA 1: Carregar do Ambiente Local
        try:
            ...
            # TENTATIVA 2 (PLANO B): Nuvem (Baixa os bytes e cria um arquivo temporário bruto)
            ...
            # Carrega o áudio temporário na memória e apaga o bruto em seguida
            snd = parselmouth.Sound(temp_raw.name)
            os.remove(temp_raw.name)
            
        # ETAPA CRUCIAL: NORMALIZAÇÃO DE PICO (Peak Normalization)
        # Escala o volume para que o pico encoste no limite superior digital (0.99 ou -0.1 dBFS).
        # Resolve o problema de pacientes gravando "muito baixo" ou "muito alto".
        snd.scale_peak(0.99)
        
        # Salva o arquivo normalizado na pasta temporária para uso dos extratores
